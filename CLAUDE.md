...
> - **버전업 정책(소유자 지정): 현재 배포 버전은 v2.4.0으로 유지.** 앞으로는 사소한 수정이라도
>   *새로 배포(빌드/머지)할 때* 패치(Z)를 올린다(다음 배포부터 v2.4.1 …). 매 배포마다 세 매니페스트
>   버전을 함께 올리고 CHANGELOG에 해당 버전 항목을 추가하며 PR 제목에도 `[vX.Y.Z]`를 붙인다.
