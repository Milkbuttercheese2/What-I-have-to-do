...
- `commands.rs` — thin `#[tauri::command]` wrappers (`load_all`, `save_all`, `save_fields`, `save_presets`, `save_id_kinds`, `save_settings`, `backup_export`, `backup_import`, …) that just call into `db::*` and map errors to `String`, plus the global mini-capture shortcut: `CAPTURE_SHORTCUT` is **fixed to Ctrl+Alt+Space since v2.31** (no runtime re-binding; the old `set_capture_shortcut`/`set_autostart`/`save_recur_defs` commands were removed). v3.0.0 adds `pick_file_path`/`open_file_path`/`reveal_file_path` (file links, via the dialog/opener plugins) and `everything_search` (hand-rolled HTTP/1.0 GET to a locally running Everything's HTTP server on 127.0.0.1 — returns the raw JSON body; parsing stays in `src/everything.js`; unit-tested against a fake TCP server in commands.rs's test module). Business logic does **not** belong here or in `db/` — it stays in the frontend JS (see Migration-in-progress note above); Rust is CRUD-only.
...
- `recur.js` (v3.1.0) — pure next-occurrence math for 주기 업무 (`{type:"dow"|"every"}` on `it.recur`, stored in `items.recur` TEXT via migration 004); completing a recurring item archives a done copy and re-arms the original (`completeOccurrence` in state.js, wired in render.js chk handler).
- `settings-menu.js` (v3.1.0) — header [설정] dropdown that hosts 저장 위치/백업/불러오기/XLSX/프리셋 관리 buttons (ids unchanged — action listeners stay in their own modules) plus the two Everything toggles (`settings.everythingAutostart`/`everythingQuickSearch`).
...
- `everything.js` (v3.0.0) — optional Everything(voidtools) integration: board `#search` input also queries a locally running Everything's HTTP server (via the `everything_search` Rust command, 127.0.0.1 only; tries ports 80→8080 unless `settings.everythingPort` is set) and shows file results in the `#ev-results` strip. Fully non-invasive: connection failure hides the strip and backs off 2 minutes. Result clicks reuse render.js's delegated `data-fopen` handler.
