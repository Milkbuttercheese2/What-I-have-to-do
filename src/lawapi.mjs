...
function normalizeArticles(raw) {
  ...
  for (const a of asArray(raw)) {
    ...
    // 항 아래 호·목까지 이어붙인다. 항내용만 취하면 각 호의 실제 규정이 누락된다.
    const paras = asArray(findKey(a, "항"))
      .map((p) => {
        const lead = clean(flattenText(findKey(p, "항내용") ?? ""));
        const hos = asArray(findKey(p, "호"))
          .map((h) => clean(flattenText(findKey(h, "호내용") ?? h)))
          .filter(Boolean);
        const joined = [lead, ...hos].filter(Boolean).join("\n");
        return joined || clean(flattenText(p));
      })
      .filter(Boolean);
...
export async function fetchAdminRule({ seq, ruleId }) {
  ...
  // 별표·서식은 조문 텍스트에 담기지 않는다. 표 내용(있으면) + HWP/PDF 링크(kordoc 대상)를 모은다.
  const 별표 = collectByulpyo(json);

  // ★ "무조건 다 와야 한다": 조문 구조가 없는 문서(공고·고시·별표전용)도 버리지 않는다.
  //   통짜 본문이 있으면 단일 조문으로, 그마저 없으면 별표 참조 스텁으로 노드를 남긴다.
  if (articles.length === 0) {
    const refs = 별표.map((a) => a.title).filter(Boolean).join(", ");
