...
export const S = {
  ...
  loaded: false,
  /* v3.6.0 증분 저장 — **DB 에 아직 반영되지 않은 완료 업무의 id.**
     저장은 이제 완료 업무 행을 건드리지 않는다(그래야 저장 비용이 누적 업무 수가 아니라
     지금 다루는 업무 수에만 비례한다). 그래서 완료 업무에 생긴 변화는 여기에 적어 둬야
     저장에 실린다. 적히는 경우는 딱 둘 — **완료 체크/되살리기(toggleDone)** 와
     **완료 업무 삭제**. 그 밖의 경로는 완료 업무를 건드리지 않는다(양식은 읽기 전용,
     메모 합치기는 완료 업무를 거부, 주기 생성은 새 미완료 업무).
     store.js 가 저장에 성공한 배치의 id 만 지운다(실패하면 남겨 재시도에 다시 실린다). */
  doneDirty: new Set(),
...
/* ⚠️ `it.done` 이 바뀌는 곳은 코드 전체에서 여기 하나뿐이다(v3.6.0 증분 저장이
   기대는 성질 — 그래서 완료 업무의 변화를 여기서만 표시해도 새는 곳이 없다).
   완료로 갈 때든 되살릴 때든 id 를 적어 둬야 그 변화가 저장에 실린다. */
export function toggleDone(it){
  it.done = !it.done;
  it.doneAt = it.done ? Date.now() : null;
  S.doneDirty.add(it.id);
  return it;
}

/* 완료 업무를 지웠다는 사실을 저장에 알린다 — 저장은 완료 업무를 다시 쓰지 않으므로
   '목록에 없다'만으로는 *지웠다* 와 *원래 안 보내는 완료 업무다* 를 구분할 수 없다.
   미완료 업무는 저장이 통째로 지우고 다시 넣으므로 표시가 필요 없다. */
export function markDoneRemoved(it){
  if(it && it.done) S.doneDirty.add(it.id);
}
