...
- **전역 단축키 미니 창**: 다른 프로그램을 쓰는 중에도 **Ctrl+Alt+Space**(고정)로 미니 창을 띄워 **내 업무 검색**, **Alt**로 빠른 메모 전환 후 Ctrl+Enter로 등록 (v2.23; 이후 내용 보존·초안 자동 등록, v2.5.21부터 검색이 기본 모드)
- **다크 / 화이트 모드 (v2.6.0)**: 메인 화면과 미니 창의 밝기를 **각각** 선택 (설정 팝업). 미니 창의 시작 화면(검색/메모)과 `Ctrl+Enter` 동작(바로 등록/양식 열기)도 설정에서 선택
