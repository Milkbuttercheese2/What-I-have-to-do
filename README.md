...
전화받고 잊어버리기 전에 던져두는 업무 메모 보드. 공무원 내부망(인터넷 차단, 관리자 권한 없는 PC) 환경을 위한 **오프라인 단독 실행형 Windows 데스크톱 앱**입니다. (v3.0.0부터 이름이 '뭐해야 했더라' → **'뭐하려 했더라'**로 바뀌었고, 버전도 X.Y.Z 체계를 씁니다. 기존 데이터·백업과 완전 호환됩니다.)

> 📖 **처음 사용하시나요?** 사용자용 안내는 [`사용 설명서.md`](./사용%20설명서.md)에 친절하게 정리돼 있습니다. 아래는 요약과 개발자용 정보입니다.

- 바로 입력(자유 메모) 또는 양식 입력(관련인·식별번호·세부할일·파일 링크·마감시각·반복)
- **전역 단축키 미니 캡처 창**: 다른 프로그램을 쓰는 중에도 **Ctrl+Alt+Space**(고정)로 메모창을 띄워 Ctrl+Enter로 등록 (v2.23; v3.1.0부터 내용 보존·초안 자동 등록·Alt 검색 모드)
