...
pub fn load_phonebook(conn: &Connection) -> DbResult<Vec<PhonebookEntry>> {
    let mut stmt =
        conn.prepare("SELECT id, who, org, phone FROM phonebook ORDER BY sort_order, id")?;
...
pub fn save_phonebook_tx(tx: &Transaction, entries: &[PhonebookEntry]) -> DbResult<()> {
    tx.execute("DELETE FROM phonebook", [])?;
    {
        ...
        let mut ins = tx.prepare(
            "INSERT OR IGNORE INTO phonebook (id, who, org, phone, sort_order) VALUES (?1, ?2, ?3, ?4, ?5)",
        )?;
        for (i, e) in entries.iter().enumerate() {
            ins.execute(params![e.id, e.who, e.org, e.phone, i as i64])?;
        }
    }
    Ok(())
}
...
pub fn search_phonebook(
    ...
    let mut stmt = conn.prepare(
        "SELECT id, who, org, phone FROM phonebook
