...
test("파일이 있으면 PDF iframe 을 본체로 건다", () => {
  ...
  const html = AnnexView.render(별표, { avail, esc });
...
test("파싱표(罫線 → HTML)는 어떤 경우에도 그리지 않는다", () => {
  const withFile = AnnexView.render(별표, { avail: new Set(["160399157.pdf"]), esc });
...
test("파일만 있는 별지 서식(텍스트 없음)도 뷰어로 연다", () => {
  ...
  const html = AnnexView.render(서식, { avail: new Set(["222.pdf"]), esc });
